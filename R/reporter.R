...
#' }
make_report <- function(download_log, path) {
  result_table = data.frame("ts_name" = character(), "comment" = character(),"freq" = integer(), 
  ...
  for(file_num in 1:nrow(download_log)) {
    ...
    for(ts_num in 1:ts_count) {
      cur_ts <- df_ts[,ts_num]
      cur_ts_log = data.frame("ts_name" = NA, "comment" = NA,
                              "freq" = NA, 
...
#' }
raise_warnings <- function(report_table) {
  ...
  for(ts_num in 1:nrow(report_table)) {
    ...
    if(!is.na(!ts["numeric_values"]) & !as.logical(ts["numeric_values"])) {
      warning <- template
      warning_message = sprintf("%s has non-numeric objects", 
    ...
    diff_in_freq = NA
    delta = NA
    if(freq == 4) {diff_in_freq = as.double(diff_in_quarters); delta = 1}
    if(freq == 12) {diff_in_freq = as.double(diff_in_months); delta = 2}
    if(freq == 7) {diff_in_freq = as.double(diff_in_weeks); delta = 1}
    if(freq == 365) {diff_in_freq = as.double(diff_in_days); delta = 7}
...
#report_table <- make_report(download_log_new, path_day)
#warnings_table <- raise_warnings(report_table)
#report_table$corr_t_t.freq
#report_table$MAPE_naive
#report_table$MAPE_Snaive
#make_report(download_log_new, path_day)
